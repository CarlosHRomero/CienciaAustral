...
namespace Ciencia.BLL
{
    class EvolUltimaCar
    {
        ...
        {
            car_evolUManager man = new car_evolUManager();
            var lista = man.Seleccionar("", "");
            int i = 0;
            foreach (Car_evol_u reg in lista)
            {
        ...
        List<PropertyInfo> campos = typeof(Car_evol_u).GetProperties().ToList();
        private void ActualizarRegistro(Car_evol_u regDes)
        {
            List<clsEvolucion> listaEvol;
            string where = string.Format("Evol_Ingr_Id = {0} ", regDes.Evol_Ingr_Id_u.ToString());
            string order = "evol_f desc";
        ...
        {
            if (listaEquiv == null)
                listaEquiv = man.Seleccionar("tablaId =101", "", "");
        ...
        void ActualizarCampo(List<clsEvolucion> lista, PropertyInfo campo, CienciaEquiv ceCampo, Car_evol_u regDes)
        {
